...
class Manage extends SharedController
{
    ...
    public function editAction()
    {
        ...
        $userdob = $user->getDOB();
        $tmpdob  = explode('-', $userdob);
        $dob     = "{$tmpdob[1]}/{$tmpdob[2]}/{$tmpdob[0]}";

        return $this->render('UserModule:manage:edit.html.php', compact('user','dob'));
    ...
    public function editsaveAction()
    {
        ...
        $missingFields = array();
        $post          = $this->post();
        $requiredKeys  = array('userTitle', 'userFirstName', 'userLastName', 'userDOB', 'userEmail', 'userPrimaryPhone');
        ...
        // transform date into a db friendly one.
        $tmpdob = explode('/', $post['userDOB']);
        ...
        $userStorage->update(
            array(
                ...
                'dob'             => $dob,
                'primary_phone'   => $post['userPrimaryPhone'],
                'twitter'         => $post['userTwitter'],
                'facebook'        => $post['userFacebook'],
            ),
